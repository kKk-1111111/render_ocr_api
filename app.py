...
SCANNED_PAGE_DPI = int(os.getenv("SCANNED_PAGE_DPI", "120"))
...
PAGE_OCR_TIMEOUT_SECONDS = int(os.getenv("PAGE_OCR_TIMEOUT_SECONDS", "60"))
TESSERACT_TIMEOUT_SECONDS = int(os.getenv("TESSERACT_TIMEOUT_SECONDS", "45"))
OCR_WORKER_RECYCLE_PAGES = int(os.getenv("OCR_WORKER_RECYCLE_PAGES", "1"))
STORE_PAGE_DETAILS = os.getenv("STORE_PAGE_DETAILS", "false").lower() in (
    "1",
    "true",
    "yes",
)
...
OCR_FORCE_TESSERACT = os.getenv("OCR_FORCE_TESSERACT", "false").lower() in (
    "1",
    "true",
    "yes",
)
...
def dependencies() -> dict[str, Any]:
    ...
    try:
        _, rapidocr_module = load_rapidocr_class()
    ...
    return {
        ...
        "rapidocr_module": rapidocr_module,
        ...
        "ocr_force_tesseract": OCR_FORCE_TESSERACT,
        "max_consecutive_ocr_failures": MAX_CONSECUTIVE_OCR_FAILURES,
        "store_page_details": STORE_PAGE_DETAILS,
...
_rapidocr_module_name: Optional[str] = None


def load_rapidocr_class() -> tuple[Any, str]:
    errors: list[str] = []
    for module_name in ("rapidocr_onnxruntime", "rapidocr"):
        try:
            ...
            rapidocr_class = getattr(module, "RapidOCR")
            return rapidocr_class, module_name
        except Exception as exc:
            errors.append(f"{module_name}: {type(exc).__name__}: {exc}")
    raise RuntimeError("RapidOCR import failed. " + " | ".join(errors))


def rapidocr_image(image_path: Path) -> tuple[str, Optional[float]]:
    global _rapidocr_instance, _rapidocr_module_name
    try:
        if _rapidocr_instance is None:
            RapidOCR, _rapidocr_module_name = load_rapidocr_class()
        ...
        module_note = f" via {_rapidocr_module_name}" if _rapidocr_module_name else ""
        raise RuntimeError(f"RapidOCR failed{module_note}: {exc}") from exc
...
def ocr_rendered_image(
    ...
    deps = dependencies()
    if OCR_FORCE_TESSERACT:
        engine_order = ["tesseract"]
    elif OCR_ENABLE_ENGINE_FALLBACK:
        if engine == "tesseract":
...
def process_job(job_id: str, request: dict[str, Any]) -> None:
    ...
    try:
        ...
        data["ocr_force_tesseract"] = OCR_FORCE_TESSERACT
        ...
        for page_number in range(page_start, page_end + 1):
            ...
            try:
                ...
                data["extracted_pages"] = len(full_text_parts)
                ...
                save_job(data)
                if ocr_worker is not None and OCR_WORKER_RECYCLE_PAGES <= 1:
                    ocr_worker.stop()
                    ocr_worker = None
                gc.collect()
        ...
        data["extracted_pages"] = len(full_text_parts)
        data["failed_pages"] = len(failures)
        data["pages"] = pages if STORE_PAGE_DETAILS else []
        data["failures"] = failures
        data["page_index"] = page_index
        data["text_pages"] = sum(1 for item in page_index if item["page_type"] == "text_page")
        ...
        if stopped_early_error:
            data["status"] = "failed"
            data["error"] = stopped_early_error
        elif full_text_parts and failures:
...
def get_job_status(job_id: str, authorization: Optional[str] = Header(default=None)) -> dict[str, Any]:
    ...
    return {
        ...
        "ocr_force_tesseract": data.get("ocr_force_tesseract", OCR_FORCE_TESSERACT),
